        ...
        .factory('FileUploader', ['fileUploaderOptions', '$rootScope', '$http', '$window', '$compile',
            function (fileUploaderOptions, $rootScope, $http, $window, $compile) {
                ...
                FileUploader.prototype._xhrTransport = function (item) {
                    var xhr = item[0]._xhr = new XMLHttpRequest();
                    var form = new FormData();
                    var that = this;

                    that._onBeforeUploadItem(item[0]);

                    angular.forEach(item[0].formData, function (obj) {
                        angular.forEach(obj, function (value, key) {
                            form.append(key, value);
                        });
                    });

                    form.append('file1', item[0]._file, item[0].file.name);
                    form.append('file2', item[1]._file, item[1].file.name);

                    xhr.upload.onprogress = function (event) {
                        var progress = Math.round(event.lengthComputable ? event.loaded * 100 / event.total : 0);
                        that.onProgressAll(progress);
                    };
                    ...
                    xhr.onload = function () {
                        ...
                        that[method](item[0], response, xhr.status, headers);
                        that._onCompleteItem(item[0], response, xhr.status, headers);
                        that._onCompleteItem(item[1], response, xhr.status, headers);
                    };

                    xhr.onerror = function () {
                        var headers = that._parseHeaders(xhr.getAllResponseHeaders());
                        var response = that._transformResponse(xhr.response);
                        that._onErrorItem(item[0], response, xhr.status, headers);
                        that._onCompleteItem(item[0], response, xhr.status, headers);
                        that._onErrorItem(item[1], response, xhr.status, headers);
                        that._onCompleteItem(item[1], response, xhr.status, headers);
                    };
                    ...
                    xhr.onabort = function () {
                        var headers = that._parseHeaders(xhr.getAllResponseHeaders());
                        var response = that._transformResponse(xhr.response);
                        that._onCancelItem(item[0], response, xhr.status, headers);
                        that._onCompleteItem(item[0], response, xhr.status, headers);
                        that._onCancelItem(item[1], response, xhr.status, headers);
                        that._onCompleteItem(item[1], response, xhr.status, headers);
                    };

                    xhr.open(item[0].method, item[0].url, true);

                    xhr.withCredentials = item[0].withCredentials;

                    angular.forEach(item[0].headers, function (value, name) {
                 ...
                 * Uploads a FileItem
                 */
                FileItem.prototype.uploads = function (value) {
